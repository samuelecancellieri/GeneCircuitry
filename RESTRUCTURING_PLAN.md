# GeneCircuitry Repository Restructuring Plan
...
| Path                                  | Reason                                                                                                                           |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `genecircuitry/core/`                       | Abandoned refactoring — only `__pycache__/`, no source files                                                                     |
| `genecircuitry/preprocessing/` (subpackage) | Abandoned refactoring — only `__pycache__/`, no source files. Not to be confused with `genecircuitry/preprocessing.py` which is active |
| `genecircuitry/computation/`                | Abandoned refactoring — only `__pycache__/`, no source files                                                                     |
| `genecircuitry/pipeline/`                   | Abandoned refactoring — only `__pycache__/`, no source files                                                                     |

**Cleanup command:** `find genecircuitry/ -type d -name '__pycache__' -exec rm -rf {} + 2>/dev/null; rmdir genecircuitry/core genecircuitry/preprocessing genecircuitry/computation/celloracle genecircuitry/computation/hotspot genecircuitry/computation genecircuitry/pipeline 2>/dev/null`
...
| `genecircuitry.egg-info/`         | Build artifact — should be in `.gitignore`, not committed                                                                                                               | ✅     |
...
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------ |
| `examples/quick_demo.py`              | `from genecircuitry.preprocessing import plot_qc_violin, plot_qc_scatter` — these functions don't exist in `preprocessing.py`            | ✅     |
| `examples/example_qc.py`              | Same broken imports as above                                                                                                       | ✅     |
| `examples/test_config_integration.py` | `from genecircuitry.preprocessing import perform_grn_pre_processing` — function is in `celloracle_processing.py`, not `preprocessing.py` | ✅     |
...
| `examples/complete_pipeline.py` | `examples/`      | `genecircuitry/pipeline/controller.py`        | This is the **core pipeline engine** (1748 lines), not an "example". It's the central orchestrator imported by `run_complete_analysis.py` |
...
- **File**: `genecircuitry/grn_deep_analysis.py` (referenced on line ~79)
...
- **File**: `genecircuitry/preprocessing.py`
...
- **Bug**: Imports `plot_qc_violin`, `plot_qc_scatter` from `genecircuitry.preprocessing` (don't exist). Calls `perform_qc(adata, plot=False)` (parameter doesn't exist)
- **Fix**: Update imports to use `genecircuitry.plotting.qc_plots` and fix `perform_qc` calls

### 4.4 Broken Example Imports (3 files)

- `examples/celloracle_workflow.py` — imports `perform_grn_pre_processing` from `genecircuitry.preprocessing` (it's in `genecircuitry.celloracle_processing`)
...
- **File**: `genecircuitry/__init__.py`
- **Bug**: `from . import celloracle_processing` and `from . import hotspot_processing` run at import time, so `import genecircuitry` fails if CellOracle/Hotspot aren't installed
...
2. Dedicated `genecircuitry/plotting/` subpackage (`grn_plots.py`, `hotspot_plots.py`)

**Resolution**: After confirming `plotting/` subpackage covers all functionality, deprecate the inline plotting functions in the legacy modules. Add `# DEPRECATED: Use genecircuitry.plotting.grn_plots instead` comments.
...
**Resolution**: Move to `genecircuitry/pipeline/controller.py` and split into:
...
### 5.3 `genecircuitry.egg-info/` Committed to Repo
...
- `genecircuitry.egg-info/`
- `genecircuitry/core/` (ghost)
- `genecircuitry/preprocessing/` subpackage (ghost)
- `genecircuitry/computation/` (ghost)
- `genecircuitry/pipeline/` (ghost — will be recreated with real source)
...
- [ ] **Phase 1: Cleanup** — Remove files marked `TO REMOVE`
  - [ ] Delete ghost subpackages (`genecircuitry/core/`, `genecircuitry/preprocessing/`, `genecircuitry/computation/`, `genecircuitry/pipeline/`)
  ...
  - [ ] Delete `genecircuitry.egg-info/`, `tests/test_preprocessing/`
...
- [ ] **Phase 2: Move**
  ...
  - [ ] Move `examples/complete_pipeline.py` → `genecircuitry/pipeline/controller.py`
